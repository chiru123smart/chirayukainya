...
export default function App(){
  ...
  useEffect(()=>{if(activeYear!==null&&detailRef.current){detailRef.current.scrollIntoView({behavior:"smooth",block:"nearest"})}},[activeYear]);
    ...
    <div style={{background:NAVY,color:TEXT,fontFamily:"'Outfit',sans-serif",minHeight:"100vh"}}>
      ...
      <style>{`
        *{margin:0;padding:0;box-sizing:border-box}html{scroll-behavior:smooth}
        ...
        .year-card{flex:0 0 auto;width:150px;padding:20px 16px;border-radius:14px;cursor:pointer;transition:all .3s;border:1px solid ${ACCENT}33;background:${NAVY2};text-align:center}
        .year-card:hover{border-color:${BLUE}66;transform:translateY(-3px)}
        .year-card.active{transform:translateY(-3px)}
        .proj-chip{padding:16px 20px;border-radius:12px;cursor:pointer;transition:all .3s;border:1px solid ${ACCENT}44;background:${NAVY3}}
        .proj-chip:hover{border-color:${BLUE}66}
        ...
        .year-scroll{display:flex;gap:14;overflow-x:auto;padding:10px 0 20px;scrollbar-width:thin}
        .year-scroll::-webkit-scrollbar{height:4px}
        .year-scroll::-webkit-scrollbar-thumb{background:${ACCENT}66;border-radius:2px}
        @media(max-width:768px){
          ...
          .year-card{width:130px;padding:16px 12px}
      ...
      <section style={{background:NAVY2,borderTop:`1px solid ${ACCENT}22`,borderBottom:`1px solid ${ACCENT}22`,padding:"28px 32px"}}>
        <div className="company-bar" style={{maxWidth:700,margin:"0 auto",display:"flex",justifyContent:"center",alignItems:"center",gap:48}}>
          ...
          {[
            {name:"Reliance Retail",indices:[0,1,2,3]},
            {name:"Flipkart",indices:[4]},
            {name:"Myntra",indices:[5]},
            {name:"Infosys",indices:[6]},
          ].map((c,i)=>(
            <span key={i} style={{fontSize:15,fontWeight:600,color:TEXT,cursor:"pointer",transition:"color .3s",borderBottom:`2px solid transparent`}}
              ...
              onClick={()=>{setActiveYear(c.indices[0]);setActiveProject(null);go("work");}}
            >{c.name}</span>
          ))}
        </div>
      </section>
      ...
      <section id="work" className="section-pad" style={{padding:"100px 32px",background:NAVY2}}>
        <div style={{maxWidth:1200,margin:"0 auto"}}>
          ...
          <div className="year-scroll" style={{display:"flex",gap:14,overflowX:"auto",paddingBottom:20}}>
            {timeline.map((era,i)=>(
          ...
          {activeYear!==null&&(
            <div ref={detailRef} style={{animation:"fadeIn .3s ease-out",marginTop:8}}>
              {(()=>{
                const era=timeline[activeYear];
                return(
                  <div style={{background:NAVY,border:`1px solid ${era.color}33`,borderRadius:16,padding:"32px",marginBottom:16}}>
                    {/* Era header */}
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:12,marginBottom:8}}>
                      <div>
                        <span style={{fontSize:22,fontWeight:700,color:TEXT}}>{era.role}</span>
                    ...
                    <div className="proj-grid" style={{display:"grid",gridTemplateColumns:`repeat(auto-fit, minmax(300px, 1fr))`,gap:14}}>
                      {era.projects.map((p,pi)=>{
                        const isOpen=activeProject===`${activeYear}-${pi}`;
                        return(
                          <div key={pi} className="proj-chip"
                            onClick={()=>setActiveProject(isOpen?null:`${activeYear}-${pi}`)}
                            style={{borderColor:isOpen?era.color:undefined,gridColumn:isOpen?"1 / -1":undefined}}>
                            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                              <span style={{fontSize:15,fontWeight:600,color:TEXT}}>{p.title}</span>
                              <span style={{fontSize:16,color:DIM,transition:"transform .3s",transform:isOpen?"rotate(45deg)":"none",flexShrink:0,marginLeft:12}}>+</span>
                            </div>
                            {!isOpen&&<p style={{fontSize:13,color:DIM,marginTop:8,lineHeight:1.5}}>{p.desc.length>140?p.desc.slice(0,140)+"...":p.desc}</p>}
                            {isOpen&&(
                              <div style={{animation:"fadeIn .3s ease-out",marginTop:14}}>
                                <p style={{fontSize:14,color:DIM,lineHeight:1.7,marginBottom:16}}>{p.desc}</p>
                                {p.details.map((d,j)=>(
                                  <div key={j} style={{display:"flex",gap:10,marginBottom:8}}>
                                    <span style={{color:era.color,fontSize:13,marginTop:2,flexShrink:0}}>▸</span>
                                    <span style={{fontSize:13,color:DIM,lineHeight:1.6}}>{d}</span>
                                  </div>
                                ))}
                                <div style={{marginTop:14,padding:"10px 14px",background:`${era.color}10`,border:`1px solid ${era.color}25`,borderRadius:8}}>
      ...
      <section id="contact" className="section-pad" style={{padding:"100px 32px",textAlign:"center"}}>
        <div style={{maxWidth:600,margin:"0 auto"}}>
          ...
          <p style={{fontSize:16,color:DIM,lineHeight:1.8,marginBottom:36}}>Open to new opportunities, partnerships, advisory roles, and mentoring. Whether you're building in retail tech, scaling operations with AI, or just want to exchange ideas.</p>
          ...
          <div style={{flex:1,overflow:"auto",padding:14,display:"flex",flexDirection:"column",gap:10}}>
            {msgs.map((m,i)=>(
              <div key={i} style={{display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start"}}>
                <div className="chat-msg" style={{background:m.role==="user"?BLUE:NAVY2,color:m.role==="user"?"#fff":TEXT,borderBottomRightRadius:m.role==="user"?4:16,borderBottomLeftRadius:m.role==="user"?16:4,border:m.role==="user"?"none":`1px solid ${ACCENT}33`}}>{m.content}</div>
              </div>
            ))}
